...
function  TCompareRightThread.InitODBC(const connectionstring : string): boolean;
 begin
  Assert(connectionstring <> '', 'TCompareRightThread.InitODBC :: invalid connection string');
...
begin
  ...
  CompareData[XRightSide].Data[XFileCount]   := 0;
  CompareData[XRightSide].Data[XFolderCount] := 0;
  CompareData[XRightSide].Data[XFileSize]    := 0;
...
begin
  ...
  try
    ...
    while not XQuery.eof do begin
      ...
      inc(CompareData[XRightSide].Data[XFileSize], StrToInt64(XQuery.FieldByName('FileSize').AsString));

      if XQuery.FieldByname('Directory').AsInteger = 1 then // is folder
        inc(CompareData[XRightSide].Data[XFolderCount])
      else
        inc(CompareData[XRightSide].Data[XFileCount]);
  ...
  try
    ...
    if not lResults.IsEmpty then begin
      ...
      while not lResults.Eof do begin
        ...
        inc(CompareData[XRightSide].Data[XFileSize], lResults.Fields[fFileSize].AsLargeInt);
