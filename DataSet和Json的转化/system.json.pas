...
uses
  Classes, DateUtils, SysConst, StrUtils, Character, JSONConsts;
...
begin
  if (FJsonString <> nil) and FJsonString.GetOwned then
...
begin
  inherited Create;
  FStrBuffer := TStringBuilder.Create;
  if System.Length(Value) > FStrBuffer.Capacity then
    FStrBuffer.Capacity := System.Length(Value);
  FStrBuffer.Append(Value);
end;
