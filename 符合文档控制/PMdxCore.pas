...
//  TRecordBuffer = PAnsiChar;
  ...
  TdxLocalizationTranslateResStringEvent = procedure(const AResStringName: string; AResString: Pointer) of
    object;
  ...
  public
    ...
    property OnTranslateResString: TdxLocalizationTranslateResStringEvent read FOnTranslateResString write
      FOnTranslateResString;
  end;
...
function WriteBufferToStream(AStream: TStream; ABuffer: Pointer; ACount: Longint; var RealSize: Integer):
  Longint;
function WriteCharToStream(AStream: TStream; AValue: AnsiChar; var RealSize: Integer): Longint;
function WriteDoubleToStream(AStream: TStream; AValue: Double; var RealSize: Integer): Longint;
function WriteIntegerToStream(AStream: TStream; AValue: Integer; var RealSize: Integer): Longint;
function WriteSmallIntToStream(AStream: TStream; AValue: SmallInt; var RealSize: Integer): Longint;
function WriteStringToStream(AStream: TStream; const AValue: AnsiString; var RealSize: Integer): Longint;
...
function dxCharIsAlpha(Ch: Char): Boolean;
begin
  Result := {$IFDEF DELPHI12}dxWideIsAlpha{$ELSE}dxAnsiIsAlpha{$ENDIF}(AnsiChar(Ch));
end;
...
function dxCharIsNumeric(Ch: Char): Boolean;
begin
  Result := {$IFDEF DELPHI12}dxWideIsNumeric{$ELSE}dxAnsiIsNumeric{$ENDIF}(AnsiChar(Ch));
end;
...
begin
  ...
  begin
    ...
    MultiByteToWideChar(ACodePage, MB_PRECOMPOSED, PAnsiChar(ASource), ASrcLength, PWideChar(Result),
      ADestLength);
  end;
end;
...
begin
  ...
  begin
    ...
    SetLength(Result, ADestLength);
    WideCharToMultiByte(ACodePage, 0, PWideChar(ASource), ASrcLength, PAnsiChar(Result), ADestLength, nil,
      nil);
  end;
end;
...
constructor TdxProductResourceStrings.Create(const AName: string; AInitializeProc:
  TdxAddResourceStringsProcedure);
begin
  FName := AName;
...
procedure TdxResourceStringsRepository.RegisterProduct(const AProductName: string; AAddStringsProc:
  TdxAddResourceStringsProcedure);
begin
